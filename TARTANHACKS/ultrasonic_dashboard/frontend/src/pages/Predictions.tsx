...
export function Predictions(): JSX.Element {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const nextSchedule = await getSchedule(90);
          ...
          <table>
            ...
            <tbody>
              {accuracyRows.map((row) => (
                <tr key={row.id}>
                  <td>{formatDateTimeMinutes(row.timestamp)}</td>
                  <td>{prettyBinNameFromId(row.bin_id)}</td>
                  <td>{row.target_fullness}%</td>
                  <td>{formatDateTimeMinutes(row.predicted_full_at)}</td>
                  <td>
